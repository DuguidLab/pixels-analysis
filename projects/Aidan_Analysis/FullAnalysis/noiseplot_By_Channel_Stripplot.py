...
depth1 = meta_spikeglx(myexp, 0) #give us data for session1, session two will be the same so perhaps we can simply clone this??
...
fig, ax = plt.subplots()
p = sns.stripplot(
    x="date", y="SDs", data=df2,
    palette="Spectral", ax=ax, hue="channel"
).set(title="Noise for Subject VR59")

ax.set_xticklabels([t.get_text().split("T")[0] for t in ax.get_xticklabels()]) #Remove zeroes from date line
