...
hits_off = myexp.align_trials(
    ...
    Events.led_off,
    "spike_rate",
    duration=8, #May need to increase this depending on how far back the LED came on
    units=unit
)
...
def event_times(event, myexp):
    ...
    for ses in myexp:
        sessiontimepoints=[] #Give an empty list to store this sessions times in before appending to the master list
        als = ses.get_action_labels() #Get the action labels for this session

        for rec_num in range(len(als)): #This will run through all recording numbers in the above action label data
            ...
            for trial in start:
                ...
                event_time = np.where(np.bitwise_and(events[trial:trial+10000], getattr(Events, event)))[0]                
...
rugdata = -rugdata/1000 #Converts the positive values to a negative second, to signify the duration before the LED off
...
for s, session in enumerate(myexp):
    ...
    plt.suptitle(
        f"session {name} - per-unit across trial firing rate, aligned to LED on"
    )
